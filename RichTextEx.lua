...
-------------------------------------------------
	...
	Fixed by HuangZhentao on 16-07-05.
-------------------------------------------------
...
一个简单的富文本 Label，用法
	...
	txt:setText("<font fonts/fzlbjt.ttf><64><#FFF><effect shadow><@F00>A ABB<img temp.png><img_50*50 temp.png>B<#00F>&lt;C\tC&gt;<@0F0><64>D\nD<18>EE<")
...
function _M:ctor(fontSize, textColor)
	...
	self._labelEffectDef = cc.LabelEffect.NORMAL
	self._labelEffect 	 = cc.LabelEffect.NORMAL
	
	self._effectColorDef = cc.c4b(255, 255, 255, 255)
	self._effectColor 	 = cc.c4b(255, 255, 255, 255)
	
	self._outlineSizeDef = 1
	self._outlineSize 	 = 1
end
	...
	while i <= len do
		...
		if c == P_BEG then	-- <
			if (not b) and (i > p) then
				...
				obj = ccui.RichElementText:create(0, self._textColor, 255,self:htmlDecode(str), self._textFont, self._fontSize,
													self._labelEffect,c3b_to_c4b(self._effectColor), self._outlineSize)
				self:pushBackElement(obj)
				self._elements[#self._elements + 1] = obj
			end
				...
				if str_byte(text, i) == P_END then	-- >
					...
					if i > p then
						...
						elseif chr == C_RST and #str == 1 then	-- reset
							...
							self._labelEffect = self._labelEffectDef
							self._effectColor = self._effectColorDef
							self._outlineSize = self._outlineSizeDef
						...
						elseif str_sub(str, 1, 7) == "effect " or str_sub(str, 1, 8) == "effect_" then
							local strTemp = str_trim(str_sub(str, 7, i - 1))
							if strTemp == "shadow" then
								self._labelEffect = cc.LabelEffect.SHADOW
							elseif strTemp == "outline" then
								self._labelEffect = cc.LabelEffect.OUTLINE
							end
					elseif chr == C_AT and (#str == 4 or #str == 7) and tonumber(str_sub(str, 2), 16) then -- effectColor
							self._effectColor = c3b_parse(str)
						elseif str_sub(str, 1, 8) == "outline " or str_sub(str, 1, 8) == "outline_" then
							local strTemp = str_trim(str_sub(str, 8, i - 1))
							self._outlineSize = tonumber(strTemp)
		...
		elseif c == C_LN or c == C_TAB then
			...
			if c == C_LN then
				...
				obj = ccui.RichElementNewLine:create(0,cc.c3b(255,255,255),255)
				self:pushBackElement(obj)
				self._elements[#self._elements + 1] = obj
			else
				obj = cc.Node:create()
...
function _M:setDefaultFont(font)
	self._textFontDef = font
	self._textFont = font
end

function _M:setDefaultShadow(c4bColor)
	self._labelEffect = cc.LabelEffect.SHADOW
	self._effectColor = c4bColor or cc.c4b(255,255,255,255)
	
	self._labelEffectDef = cc.LabelEffect.SHADOW
	self._effectColorDef = c4bColor or cc.c4b(255,255,255,255)
end

function _M:setDefaultOutline(c4bColor, nSize)
	self._labelEffect = cc.LabelEffect.OUTLINE
	self._effectColor = c4bColor or cc.c4b(255,255,255,255)
	self._outlineSize = nSize or 1
	
	self._labelEffectDef = cc.LabelEffect.OUTLINE
	self._effectColorDef = c4bColor or cc.c4b(255,255,255,255)
	self._outlineSizeDef = nSize or 1
end
